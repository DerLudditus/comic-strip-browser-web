<?php
/**
 * Comic Strip Browser - PHP Proxy for WordPress
 * 
 * This script fetches comics from GoComics.com server-side,
...
// Validate URL is from GoComics
if (strpos($url, 'gocomics.com') === false) {
    http_response_code(400);
    echo json_encode([
        'success' => false,
        'error' => 'Invalid URL - must be from gocomics.com'
    ]);
    exit;
}
...
try {
    ...
    if (strpos($html, "You've lost the trail!") !== false && strpos($html, "404") !== false) {
...
function extractComicDate($html, $requestedUrl) {
    ...
    if (preg_match('/<link[^>]*rel=["\']canonical["\'][^>]*href=["\']([^"\']+)["\']/', $html, $matches)) {
        $url = $matches[1];
        // Extract date from URL like: .../comic-name/2025/11/14
